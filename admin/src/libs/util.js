...
export const setTitle = (routeItem, vm) => {
  ...
  const siteName =
    useI18n && vm && typeof vm.$t === 'function' ? vm.$t('siteTitle') : title
  const resTitle = pageTitle ? `${siteName} - ${pageTitle}` : siteName
  window.document.title = resTitle
}

export const moment = (date) => {
  const loc =
    (typeof localStorage !== 'undefined' && localStorage.getItem('local')) ||
    'en-US'
  const dayLocale =
    loc === 'zh-CN' || (typeof loc === 'string' && loc.startsWith('zh'))
      ? 'zh-cn'
      : 'en'
  if (dayjs(date).isBefore(dayjs().subtract(7, 'days'))) {
    return dayjs(date).format('YYYY-MM-DD')
  }
  return dayjs(date).locale(dayLocale).from(dayjs())
}
